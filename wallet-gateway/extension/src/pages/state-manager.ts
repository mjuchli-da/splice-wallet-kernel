...
/**
 * State manager for the extension pages.
 * Uses localStorage for state persistence across page navigations.
 * Same pattern as the remote wallet's state-manager.
 */

class StateManager {
    static localStorageKey(item: string): string {
        return `com.splice.wallet.ext.${item}`
    }
    ...
    intendedPage = {
        get: () => this.getWithStorage('intendedPage'),
        set: (page: string) => this.setWithStorage('intendedPage', page),
